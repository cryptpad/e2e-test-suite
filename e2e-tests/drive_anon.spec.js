...
test('drive - anon - list/grid view', async ({ page, context }) => {
  try {
    ...
    await page.reload();
    await fileActions.mainFrame.getByText('Rich text - Thu, 24 April, 2025').waitFor()

    // await fileActions.driveFileTitle('Rich text').waitFor()
    await expect(fileActions.mainFrame.getByText('Rich text - Thu, 24 April, 2025')).toBeVisible();
