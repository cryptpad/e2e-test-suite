...
docNames.forEach(function(name) {
    ...
    test(`anon - drive - ${name}`, async () => {
        ...
        if (name === 'pad') {
          ...
          await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: `anon - drive > ${name}`, status: 'passed',reason: 'Can anonymously navigate to Drive and create Kanban'}})}`);
        } catch (e) {
          console.log(e);
          await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: `anon - drive > ${name}`, status: 'failed',reason: 'Can\'t anonymously navigate to Drive and create Kanban'}})}`);
    ...
    test(`user - drive > ${name}` , async () => {
        ...
        try {
          ...
          await page1.frameLocator('#sbox-iframe').getByRole('button', { name: 'File' }).click();
          await page1.frameLocator('#sbox-iframe').getByRole('button', {name: ' Move to trash', exact: true}).click();
          await expect(page1.frameLocator('#sbox-iframe').getByText('Clicking OK will move this document to your trash. Are you sure?')).toBeVisible();
          await page1.frameLocator('#sbox-iframe').getByRole('button', { name: 'OK (enter)' }).click();
          await expect(page1.frameLocator('#sbox-iframe').getByText('That document has been moved to the trash.')).toBeVisible()
