...
test('user - request and cancel to add user as contact', async () => {
  ...
  try {
    ...
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - request and cancel to add user as contact', status: 'passed',reason: 'Can request to add user as contact and cancel request'}})}`);
  } catch (e) {
    console.log(e);
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - request and cancel to add user as contact', status: 'failed',reason: 'Can\'t request to add user as contact and cancel request'}})}`);
  }  

});


test('user - chat with contacts and erase message history', async () => {
  ...
  try {
    ...
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - chat with contacts and erase message history', status: 'passed',reason: 'Can chat with contacts and erase chat history'}})}`);
  } catch (e) {
    console.log(e);
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - chat with contacts and erase message history', status: 'failed',reason: 'Can\'t chat with contacts and erase chat history'}})}`);
  }  
...
test('user - can change password', async () => {
  ...
  try {
    ...
    await page1.frameLocator('#sbox-iframe').locator('#cp-settings-change-password-current').fill('password');
    await page1.frameLocator('#sbox-iframe').getByPlaceholder('New password', { exact: true }).click();
    await page1.frameLocator('#sbox-iframe').getByPlaceholder('New password', { exact: true }).fill('newpassword');
    await page1.frameLocator('#sbox-iframe').getByPlaceholder('Confirm new password').click();
    await page1.frameLocator('#sbox-iframe').getByPlaceholder('Confirm new password').fill('newpassword');
    ...
    await page2.frameLocator('#sbox-iframe').locator('#cp-settings-change-password-current').fill('newpassword');
    await page2.frameLocator('#sbox-iframe').getByPlaceholder('New password', { exact: true }).click();
    await page2.frameLocator('#sbox-iframe').getByPlaceholder('New password', { exact: true }).fill('password');
    await page2.frameLocator('#sbox-iframe').getByPlaceholder('Confirm new password').click();
    await page2.frameLocator('#sbox-iframe').getByPlaceholder('Confirm new password').fill('password');
    ...
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - change password', status: 'passed',reason: 'Can change password'}})}`);
  } catch (e) {
    console.log(e);
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - change password', status: 'failed',reason: 'Can\'t change password'}})}`);
  }  
...
test('user - can change display name', async () => {
  ...
  try {
    ...
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - change display name', status: 'passed',reason: 'Can change display name'}})}`);
  } catch (e) {
    console.log(e);
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - change display name', status: 'failed',reason: 'Can\'t change display name'}})}`);
  }  

});


test('user - can access public signing key', async () => {
  ...
  try {
    ...
    if (key.indexOf('test-user@cryptpad.fr') !== -1) {
      await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - can access public signing key', status: 'passed',reason: 'Can access public signing key'}})}`);
    } else {
      await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'user - can access public signing key', status: 'failed',reason: 'Can\'t access public signing key'}})}`);
    }
