...
test(`test-user drive - cleanup templates`, async ({ }) => {
    ...
    try {
      ...
      await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'cleanup test-user templates', status: 'failed',reason: 'Can\'t cleanup test-user templates'}})}`);
...
test(`cleanup user notifications`, async ({ }) => {
    ...
    try {
        ...
        await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'cleanup user notifications', status: 'passed',reason: 'Can cleanup user notifications'}})}`);

    } catch (e) {
      await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'cleanup user notifications', status: 'failed',reason: 'Can\'t cleanup user notifications'}})}`);
...
test(`cleanup team membership`, async ({ }) => {
  ...
  try {
      ...
      await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'cleanup team membership', status: 'passed',reason: 'Can cleanup team membership'}})}`);

  } catch (e) {
    await page.evaluate(_ => {}, `browserstack_executor: ${JSON.stringify({action: 'setSessionStatus',arguments: {name: 'cleanup team membership', status: 'failed',reason: 'Can\'t cleanup team membership'}})}`);
