...
docNames.forEach(function(name) {
  ...
  test(`${name} - edit document owners`, async ({ }) => {
    ...
    try {
      ...
      await pageOne.frameLocator('#sbox-iframe').locator('.fa.cp-notifications-bell.fa-bell').click()
  ...
  if (name !== 'form') {
    ...
    test(`${name} - protect with and edit password`, async ({ }) => {
      ...
      try {
        ...
        await expect(page1.frameLocator('#sbox-iframe').getByText(/^The document you are trying to open no longer exists/)).toBeVisible({timeout: 30000})
        ...
        await expect(page.frameLocator('#sbox-iframe').getByText(/The document you are trying to open no longer exists/)).toBeVisible({timeout: 30000})
        ...
        await expect(page1.frameLocator('#sbox-iframe').getByText(/^The document you are trying to open no longer exists/)).toBeVisible({timeout:5000})
    ...
    try {
     ...
     await pageOne.frameLocator('#sbox-iframe').locator('.fa.cp-notifications-bell.fa-bell').click()
      ...
      if (name === 'pad') {
        ...
        await page1.frameLocator('#sbox-iframe').frameLocator('iframe[title="Editor\\, editor1"]').locator('body').waitFor()
        await page1.frameLocator('#sbox-iframe').frameLocator('iframe[title="Editor\\, editor1"]').locator('body').fill('TEST TEXT');
        await expect(page.frameLocator('#sbox-iframe').frameLocator('iframe[title="Editor\\, editor1"]').getByText('TEST TEXT')).toBeVisible()

      }

      await page.waitForTimeout(3000)
      await expect(page.frameLocator('#sbox-iframe').getByText('test-user3', { exact: true })).toBeVisible()
